...
/**
 * Immutable envelope of typed values and their egress metadata.
 *
 * <p>The envelope is the security-aware representation evaluated by the
 * policy engine before data reaches an external sink.</p>
 */
public final class EgressEnvelope {
    private final Map<Key<?>, SatelliteEntry<?>> entries;

    private EgressEnvelope(Map<Key<?>, SatelliteEntry<?>> entries) {
        this.entries = Collections.unmodifiableMap(
                new LinkedHashMap<Key<?>, SatelliteEntry<?>>(entries));
    }
    ...
    public Collection<SatelliteEntry<?>> entries() {
        return Collections.unmodifiableList(
                new ArrayList<SatelliteEntry<?>>(entries.values()));
    }

    @Override
    public String toString() {
        return "EgressEnvelope{size=" + entries.size()
                + ", keys=" + entries.keySet() + '}';
    }

    public static final class Builder {
        private final Map<Key<?>, SatelliteEntry<?>> entries =
                new LinkedHashMap<Key<?>, SatelliteEntry<?>>();
        private final Map<String, Key<?>> keysByName =
                new LinkedHashMap<String, Key<?>>();
        ...
        public <T> Builder put(Key<T> key, T value, ValueMetadata metadata) {
            ...
            if (existing != null && !existing.equals(key)) {
                throw new IllegalArgumentException(
                        "Conflicting key definition for external name '"
                                + key.getName() + "'");
            }
        ...
        public EgressEnvelope build() {
            return new EgressEnvelope(entries);
        }
    }
}
